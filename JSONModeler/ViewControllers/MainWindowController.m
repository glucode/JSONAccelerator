...
- (void)getDataButtonPressed {
    ...
    (self.wc.urlTextField).stringValue = [self.wc.urlTextField.stringValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
- (void)toggleCanGenerateFilesTo:(BOOL)canGenerateFiles {
    ...
    if (!(canGenerateFiles == NO && (self.invalidDataView).hidden == NO)) {
    ...
    if (sender == self.genFilesView) {
        ...
        if (self.attachedWindow) {
            ...
            if (error) {
                ...
                (self.errorMessageDescription).stringValue = informativeText;
